...
2. **Multi-architecture Docker image.** The Dockerfile uses `ARG TARGETARCH` to select the correct kubeconform and trivy binaries. It builds natively on both `linux/arm64` (Apple Silicon) and `linux/amd64` (standard Linux servers). Compose passes the architecture automatically. Cross-arch builds via QEMU emulation are possible but very slow (Go compilation under emulation adds 5-10x latency).
